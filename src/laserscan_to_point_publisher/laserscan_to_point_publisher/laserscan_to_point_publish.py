...
class laserscanToPointPublish(Node):
    ...
    def __init__(self):
        ...
        if (os.environ.get("RPLIDAR_TYPE")) == 'tmini':
            self.qos_profile = QoSProfile(depth=1)
            self.qos_profile.reliability = ReliabilityPolicy.BEST_EFFORT
        else: self.qos_profile = 1 
        self.sub_laser = self.create_subscription(LaserScan,"/scan",self.laserscan_callback,self.qos_profile)
        self.sacn_point_publisher = self.create_publisher(Path,'/scan_points',10)
