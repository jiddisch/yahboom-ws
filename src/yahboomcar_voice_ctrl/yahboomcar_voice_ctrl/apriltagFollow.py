...
class apriltagFollow(Node):
    ...
    def on_timer(self):
        if self.frame is not None:
            ...
            tags = self.at_detector.detect(cv.cvtColor(self.frame, cv.COLOR_BGR2GRAY), False, None, 0.025)
            tags = sorted(tags, key=lambda tag: tag.tag_id)
            self.frame = draw_tags(self.frame, tags, corners_color=(0, 0, 255), center_color=(0, 255, 0))
            if len(tags) > 0:
                tag = tags[0]
...
def main():
    rclpy.init()
    apriltag_Follow = apriltagFollow("ApriltagFollow")
    try:
        rclpy.spin(apriltag_Follow)
    except KeyboardInterrupt:
        pass
    finally:
        apriltag_Follow.cancel()
        apriltag_Follow.destroy_node()