...
class JoyTeleop(Node):
	def __init__(self,name):
		...
		self.servos1 = self.inits1 = int(os.environ.get("INIT_SERVO_S1"))
		self.servos2 = self.inits2 = int(os.environ.get("INIT_SERVO_S2"))
	...
	def user_jetson(self, joy_data):
		...
		if joy_data.buttons[7] == 1: self.servo_init()
		#RGBLight
		...
		if joy_data.buttons[14] == 1:
			if self.angular_Gear == 1.0: self.angular_Gear = 1.0 / 5
			elif self.angular_Gear == 1.0 / 5: self.angular_Gear = 1.0 
	...
	def servo_init(self):
		self.servo_angle.s1 = self.servos1 = self.inits1
		self.servo_angle.s2 = self.servos2 = self.inits2
		for i in range(3):
			self.pub_Servo.publish(self.servo_angle)
