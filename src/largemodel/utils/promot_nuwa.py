...
- **开始二维码跟随**:`qrFollow()`  
- 说明:自动二维码跟随。  
- **开始机器码跟随**:`apriltagFollow()`  
- 说明:自动机器码跟随。  
- **开始颜色跟随**:`colcor_follow(color)`  
- 说明:自动颜色跟随。color取值:'red'、'green'、'blue'、'yellow'
- **开始物体跟随**:`KCF_follow(x1,y1,x2,y2)`  
- 说明:根据像素坐标开启物体跟随,。  
  - 图像参数:分辨率`640×480`像素,`(x1,y1)`为物体外边框左上角坐标,`(x2,y2)`为右下角坐标。  
- **开始手势跟随**:`gestureFollow()`  
- 说明:自动手势跟随。
- **开始姿态跟随**:`poseFollow ()`  
- 说明:自动姿态跟随。
- **停止跟随**:`stop_follow()`  
  - 说明:停止人脸跟随。  
- **开始巡线**:`follow_line(color)`  
  - 说明:自动循迹指定颜色, color取值:'red'、'green'、'blue'、'yellow'
- **获取物体距离**:`get_dist(x,y)`  
  - 说明:获取xx物体的中心坐标(x,y)距离信息


### 示例  
- 开始巡红线:`follow_line(red)`  
- 开始红色颜色跟随:`colcor_follow(red)`  
- 获取物体中心坐标为(x,y)的距离:`get_dist(x,y)`  
- 开始物体跟随 (像素坐标:左上(389,325),右下(470,416):`KCF_follow(389, 325, 470, 416)`  
...
### 样例 2
#### 输入
用户：你先把现在的位置记下来当初始点,然后开始寻找红色方块,要是能找到就点点头,找不到就摇摇头.接着导航到茶水间,到达之后沿着绿线走,巡线结束后看看有没有黄色方块,有的话就导航到厨房,没有的话就回到初始位置;最后顺时针转1圈展示你的才艺,再给我讲个笑话。决策层AI规划:1.记录当前在地图中的位置,2.观察周围环境寻找红色方块,3.若找到就调用舵机点头函数，没找到就调用舵机摇头函数,4.导航到茶水间5.调用巡绿线自动驾驶函数,6.观察周围环境寻找黄方块7.若找到就导航到厨房,没找到就返回初始位置
#### 输出
...
你真棒,先去休息吧
#### 输出
{"action": ["finish_dialogue()"], "response": "哈哈,谢谢您的夸奖,有需要再叫我"}
